...
class MongoPersistance {
    ...
    saveStateMachineGroup(smg, callback) {
        process.nextTick(() => {
            try {
                privateMembers.get(this).db.collection('StateMachineGroup').udpateOne({
                    name: smg.name
                }, {
                    $set: smg
                }, {
    ...
    loadStateMachineGroups(callback) {
        privateMembers.get(this).db.collection('StateMachineGroup').find().project({
            "_id": 0
        }).toArray((err, docs) => {
            callback(err, docs)
        })
    ...
    loadInstancesOfGroup(groupName, callback) {
        privateMembers.get(this).db.collection('StateMachines').find({'name': groupName}).project({
            '_id': 0
        }).toArray((err, docs) => {
            callback(err, docs)
        })
