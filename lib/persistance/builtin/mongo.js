...
class MongoPersistance {
    ...
    loadStateMachineGroup(groupName, cb) {
        privateMembers.get(this).db.collection('StateMachineGroups').findOne({
            name: groupName
        },{
            fields: {
                "_id": 0
    ...
    loadInstances(smName, cb) {
        privateMembers.get(this).db.collection("Instances").find({
            smName: smName
        }).project({
            "_id": 0
        }).toArray((err, docs) => {
    ...
    loadAllowedTransitions(instanceId, cb) {
        privateMembers.get(this).db.collection("Instances").findOne({
            ...
            this.loadStateMachine(doc.smName, doc.version, (err, sm) => {
                if (err) {
                    cb(err, undefined)
                    return
                }
                cb(undefined, Object.getOwnPropertyNames(sm.states[doc.currentState].transitions))
            })
        })
    }
