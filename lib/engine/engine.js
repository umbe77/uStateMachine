...
function onExit(sourceState, sandbox) {
    if (sourceState && sourceState.onExit) {
        ...
        const context = new vm.createContext(sandbox)
        return script.runInContext(context)(require, setTimeout)
    } else {
...
class WorkflowEngine {
    ...
    static createInstance(stateMachine, data, cb) {
    ...
    static execute(options, cb) {
        ...
        process.nextTick(() => {
            ...
            if (!transition) {
                ...
                cb(new CantMoveError(stateMachine.name, instance.instanceId, transitionName), {})
                return
            }
